...
# life_v2.1.py : life game with pygame and arrays
...
def set(a,x,y,v): a[x+DIM[0]*y] = v

def drawCells(c,co):
	for x,y in itertools.product(range(1,DIM[0]-1),range(1,DIM[1]-1)):
		cc = get(co,x,y)*get(c,x,y) # co[x+DIM[0]*y]*c[x+DIM[0]*y]
...
def rndCells(a,c,s):
	...
	for i,j in itertools.product(range(1,DIM[0]-1),range(1,DIM[1]-1)):
		if random() < s:
			set(a,i,j,ALIVE) # a[i+DIM[0]*j] = ALIVE
			set(c,i,j,get(c,i,j)+1) # c[i+DIM[0]*j] += 1
		else: set(a,i,j,DEAD) # a[i+DIM[0]*j] = DEAD
	return a,c

def setCell(c,x,y):
	set(c,x//RES,y//RES,ALIVE) # c[(x//RES)+DIM[0]*(y//RES)] = ALIVE
	return c
	
def clearCell(c,x,y):
	set(c,x//RES,y//RES,DEAD) # c[(x//RES)+DIM[0]*(y//RES)] = DEAD
	return c
...
def updateCells(c,cc,co):
	...
	for i,j in itertools.product(range(1,DIM[0]-1),range(1,DIM[1]-1)):
		...
		if get(c,i,j) == ALIVE:
			if S < S2:	set(cc,i,j,DEAD)	# muore di solitudine
			elif S == S2 or S == S3:
				set(cc,i,j,ALIVE)			# rimane invariato
				set(co,i,j,get(co,i,j)+1)
			else: set(cc,i,j,DEAD)		# muore per sovrappopolazione (caso S > S3)
		elif S == S3:
			set(cc,i,j,ALIVE)			# resuscita miracolosamente
			set(co,i,j,get(co,i,j)+1)
		else: set(cc,i,j,DEAD)			# sterile (caso S != S3)
	return (cc,co)
...
def main():
	...
	while True:
		...
		NG += 1
		pygame.display.set_caption(f'Life v.2.1 (c)2020 by Lumachina Software - @_°°           GEN:{NG}')
