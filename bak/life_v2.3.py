...
# life_v2.3.py : life game with pygame and arrays

# John Conway - Game of Life

# CopyRight 2021 by Lumachina Software - @_°° Massimiliano Cosmelli (massimiliano.cosmelli@gmail.com)
...
# in questa versione implemento le matrici la libreria 'matrici'
...
def unset(a,x,y):
	a[x][y] = DEAD
	return a
def getcolor(a,c,x,y): return a[x][y]*c[x][y]
def inccolor(a,x,y):a[x][y] += 1
...
def main():
	...
	while True:
		...
		CELLS,COLORS = updateCells(CELLS,deepCopyMatrix(CELLS),COLORS) #updateCells(CELLS,CELLS[:],COLORS)
		
		NG += 1
		pygame.display.set_caption(f'Life v.2.3 (c)2020 by Lumachina Software - @_°°           GEN:{NG}')
